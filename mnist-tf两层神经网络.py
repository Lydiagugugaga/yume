...
W1 = tf.get_variable('W1',[784,64],initializer=tf.contrib.layers.xavier_initializer())
b1 = tf.get_variable('b1',[64],initializer=tf.zeros_initializer())
W2 = tf.get_variable('W2',[64,10],initializer=tf.contrib.layers.xavier_initializer())
...
with tf.Session() as sess:
    ...
    # run trainer和cost,同时要把X、Y给feed进去：
    ...
    # 计算训练集和测试集上的准确率：
    predictions = tf.equal(tf.argmax(tf.transpose(Z2)),tf.argmax(tf.transpose(Y)))
